 ...
 */
struct [[eosio::table("config"), eosio::contract("betbos")]] contr_config {
    ...
    // Number of bets to be elected for each election count.
    uint8_t numelected = 5;

    // account to have active auth set with all all bets on the newtenure.
    name authaccount = name{0};

    // required number of bets required to approve different levels of authenticated actions.
    uint8_t auth_threshold_bets;
 ...
 */
struct [[eosio::table("candidates"), eosio::contract("betbos")]] candidates_row {
    name candidate_name;
    asset locked_tokens;
 ...
 * - `total_votes` - Tally of the number of votes cast to a bet when they were elected in. This is updated as part of the `newtenure` action.
 */
struct [[eosio::table("bets"), eosio::contract("betbos")]] bets_row {
    name bet_name;

    uint64_t primary_key() const { return bet_name.value; }
};

typedef multi_index<"bets"_n, bets_row> bets_table;

/**
 * - `candidate_name` (name) - Account name of candidate
 * - `bio` (string JSON) - Bio of candidate
 */
struct [[eosio::table("bios"), eosio::contract("betbos")]] bios_row {
    name candidate_name;
    string bio;
 ...
 */
struct [[eosio::table("votes"), eosio::contract("betbos")]] votes_row {
    name voter;
    name proxy;
    uint64_t staked;
 ...
 */
struct [[eosio::table("votejson"), eosio::contract("betbos")]] votejson_row {
    name voter;
    string vote_json;
...
public:
    ...
    betbos( name s, name code, datastream<const char*> ds )
        :contract(s,code,ds),
     ...
     * - numelected(uint16_t) : The number of candidates to elect for bets. This is used for the payment amount to bets for median amount.
     * - authaccount(name) : The managing account that controls the BOS bet permission.
     * - auth_threshold_bets (uint8) : The number of bets required to approve an action in the low permission category ( ordinary action such as a worker proposal).
     */
    [[eosio::action]]
     ...
     * @param quantity
     * @param memo A string to attach to a transaction. For staking this string should match the name of the running contract eg "bet.bos". Otherwise it will be regarded only as a generic transfer to the account.
     ...
     * - The candidate has enough staked if they are re-nominating as a candidate and the required stake has changed since they last registerd.
     *
     ...
     */
    [[eosio::action]]
    void registercand( const name cand);

    /**
     * This action is used to withdraw a candidate from being active for bet elections.
     *
     * ### Assertions:
     * - The account performing the action is authorised.
     * - The candidate is already a registerd candidate.
     *
     * @param cand - The account id for the candidate nominating.
     *
     *
     * ### Post Condition:
     * The candidate should still be present in the candidates table and be set to inactive. If the were recently an elected bet there may be a time delay on when they can unstake their tokens from the contract. If not they will be able to unstake their tokens immediately using the unstake action.
     */
    [[eosio::action]]
    void withdrawcand( const name cand);

    /**
     * This action is used to remove a candidate from being a candidate for bet elections.
     *
     * ### Assertions:
    ...
    /**
     * This action is used to resign as a bet.
     *
     * ### Assertions:
     * - The `bet` account performing the action is authorised to do so.
     * - The `bet` account is currently an elected bet.
     *
     * @param bet - The account id for the candidate nominating.
     *
     *
     * ### Post Condition:
     * The bet will be removed from the active bets and should still be present in the candidates
     ...
     *
     * A replacement bet will selected from the candidates to fill the missing place (based on vote ranking)
     * then the auths for the controlling BOS auth account will be set for the bet board.
     */
    [[eosio::action]]
    void resign( const name bet );

    /**
     * This action is used to remove a bet.
     *
     * ### Assertions:
     * - The action is authorised by the mid level of the auth account (currently elected bet board).
     * - The `bet` account is currently an elected bet.
     *
     * @param bet - The account id for the candidate nominating.
     *
     *
     * ### Post Condition:
     * The bet will be removed from the active bets and should still be present in the candidates table but will be set to inactive.
     ...
     * that time has passed. A replacement bet will selected from the candidates to fill the missing place (based on vote ranking)
     * then the auths for the controlling BOS auth account will be set for the bet board.
     */
    [[eosio::action]]
    void firebet( const name bet );
    ...
    /**
     * This action is to facilitate voting for candidates to become bets of BOS.
     * Each member will be able to vote a configurable number of bets set by the contract configuration.
     ...
     *
     * - Allocate bets from the candidates tables based on those with most votes at the moment this action is run. -- This action removes and selects a full set of bets each time it is successfully run selected from the candidates with the most votes weight. If there are not enough eligible candidates to satisfy BOS config numbers the action adds the highest voted candidates as bets as long their votes weight is greater than 0. At this time the held stake for the departing bets is set to have a time delayed lockup to prevent the funds from releasing too soon after each bet has been in office.
     * - Distribute pay for the existing bets based on the configs into the pending pay table so it can be claimed by individual candidates. -- The pay is distributed as determined by the median pay of the currently elected bets. Therefore all elected bets receive the same pay amount.
     * - Set BOS auths for the intended controlling accounts based on the configs thresholds with the newly elected bets. This action asserts unless the following conditions have been met:
     ...
     * - After the initial vote quorum percent has been reached subsequent calls to this action will require a minimum of `vote_quorum_percent` to vote for the votes to be considered sufficient to trigger a new period with new bets.
     *
     * ##### Parameters:
     *
     * candidates - bet candidates to be registerd as bets
     ...
     *
     * ### Assertions:
     * - The candidate was a registerd candidate at some point in the passed.
     * - The candidate is not already a registerd candidate.
...
private: // Private helper methods used by other actions.
    ...
    void remove_bet( const name bet );

    void invalid_candidate( const name cand, const bool lockupStake );

    void allocate_bets( const vector<name> candidates );
