void betbos::vote( const name voter, const vector<name> candidates, const string& vote_json ) {
    require_auth( voter );
    ...
    check(candidates.size() == 25, "ERR::VOTE_MIN_VOTES_REQUIRED::Must vote for  25 candidates.");
...
void betbos::unvote( const name voter ) {
    require_auth( voter );
    remove_voter( voter );
}

void betbos::remove_voter( const name voter ) {
