...
void betbos::updateVoteWeight(name bet, int64_t weight) {
    if (weight == 0) {
        print("\n Vote has no weight - No need to continue.");
    }

    auto candItr = registered_candidates.find(bet.value);
    if (candItr == registered_candidates.end()) {
        eosio::print("Candidate not found while updating from a transfer: ", bet);
        return; // trying to avoid throwing errors from here since it's unrelated to a transfer action.?!?!?!?!
    }

    registered_candidates.modify(candItr, bet, [&](auto &c) {
        c.total_votes += weight;
        eosio::print("\nchanging vote weight: ", bet, " by ", weight);
    });
}

void betbos::updateVoteWeights(const vector<name> &votes, int64_t vote_weight) {
    for (const auto &bet : votes) {
        updateVoteWeight(bet, vote_weight);
    }

    _currentState.total_votes_on_candidates += votes.size() * vote_weight;
}

void betbos::modifyVoteWeights(name voter, vector<name> newVotes) {
